...
def pd_every_view_ans_eigen_to_csv(ans_eigen_non_negative_path, views_information_path):
    ...
    csv_path = '../../experiment/us/every_ans_csv/'
    for key, value in view_ans_pd_dict.items():
        ...
        print(value)
        value.to_csv(csv_path + str(key) + '.csv')
...
def pd_views_query_eigen_to_csv(ans_eigen_non_negative_path, views_information_path):
    ...
    view_ans_pd_dict = get_pd_every_view_ans_eigen(ans_eigen_non_negative_path)
    csv_path = '../../experiment/us/views_query_information.csv'
    for key, value in view_ans_pd_dict.items():
        tau, dlsq = get_view_tau_dlsq(views_information_path, key)
        view_list.append(key)
        dlsq_list.append(dlsq)
        tau_list.append(tau)
        mean_absolute_error_list.append(value['absolute_error'].mean())
        mean_relative_error_list.append(value['relative_error'].mean())
    data = pd.DataFrame(
        {'views': view_list, 'dlsq': dlsq_list, 'tau': tau_list, 'mean absolute error': mean_absolute_error_list,
         'mean relative error': mean_relative_error_list})
...
def pd_all_ans_eigen_to_csv(path):
    ...
    denominator[denominator < min_denominator] = min_denominator
    all_ans_pd['relative_error'] = abs(all_ans_pd['real_ans'] - all_ans_pd['privacy_ans']) / denominator
    all_ans_pd.to_csv('../../experiment/us/all_ans_eigen.csv')
    print('all absolute error', all_ans_pd['absolute_error'].mean())
    print('all relative error', all_ans_pd['relative_error'].mean())
